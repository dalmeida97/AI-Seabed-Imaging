...
model_file = 'REVOL_FULL-sizeequal.h5'
...
# Ensure only 947 random images are selected for training
total_images_to_select = 947
all_image_filenames = os.listdir(train_folder)  # List all files in the train_folder
selected_image_filenames = np.random.choice(all_image_filenames, total_images_to_select, replace=False)  # Randomly select 947 filenames

# Filter the train_annotations DataFrame to only include rows with the selected filenames
train_annotations = train_annotations[train_annotations['filename'].isin(selected_image_filenames)]
...
# New Code Begins Here
# Ensure only 150 random images are selected for validation
total_validation_images_to_select = 150
all_validation_image_filenames = os.listdir(validation_folder)  # List all files in the validation_folder
selected_validation_image_filenames = np.random.choice(all_validation_image_filenames, total_validation_images_to_select, replace=False)  # Randomly select 150 filenames

# Filter the validation_annotations DataFrame to only include rows with the selected filenames
validation_annotations = validation_annotations[validation_annotations['filename'].isin(selected_validation_image_filenames)]
...
checkpoint_filepath = 'Best_REVOL_FULL-sizeequal.h5'
